...
resource "google_compute_disk" "gcp_sap_hana_boot" {
...
resource "google_compute_instance" "gcp_sap_hana" {
  ...
  boot_disk {
    ...
    source      = google_compute_disk.gcp_sap_hana_boot.self_link
  ...
  network_interface {
    ...
    network_ip         = google_compute_address.sap_hana_internal_ip.address
